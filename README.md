...
[![Build Status](https://travis-ci.org/css-modules/my-css-modulesify.svg?branch=master)](https://travis-ci.org/css-modules/my-css-modulesify)
...
First install the package: `npm install --save my-css-modulesify`

Then you can use it as a browserify plugin, eg: `browserify -p [ my-css-modulesify -o dist/main.css ] example/index.js`
...
b.add('./main.js');
b.plugin(require('my-css-modulesify'), {
  rootDir: __dirname,
...
- `global`: optional boolean. Set to `true` if you want `my-css-modulesify` to apply to `node_modules` as well as local files. You can read more about it in the [browserify docs](https://github.com/substack/node-browserify/#btransformtr-opts).
...
- Option B: configure the `jsonOutput` option with a file path and `my-css-modulesify` will generate a JSON manifest of classnames.
...
You can override the default PostCSS Plugins (and add your own) by passing `--use|-u` to `my-css-modulesify`.
...
```
browserify -p [my-css-modulesify \
...
If you set `NODE_ENV=production` then `my-css-modulesify` will generate shorter (though less useful) classnames.
