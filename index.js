...
var Core = require('my-css-modules-loader-core');
...
module.exports = function (browserify, options) {
  ...
  transformOpts.verbose = options.verbose || false;
  ...
  if (!loadersByFile[cssOutFilename]) {
    loadersByFile[cssOutFilename] = new FileSystemLoader(rootDir, plugins, { verbose: transformOpts.verbose });
  }

  // TODO: clean this up so there's less scope crossing
  Cmify.prototype._flush = function (callback) {
    
    const cssmodulesifytime = process.hrtime();
    ...
    /*
    if(self._verbose) {
      console.log(`${colors.yellow("css-modulesify")}-${colors.magenta("flush")}:${colors.green(filename)}`);
    }
    */
    ...
    loader.fetch(relFilename, '/').then(function (tokens) {
      ...
      if(self._verbose) {
        var hrend = process.hrtime(cssmodulesifytime);
        console.log(`${colors.yellow("css-modulesify")}-${colors.magenta("  flush")}: ${colors.green(self._filename)} -> ${ `${colors.cyan(hrend[1]/1000000)}ms` }}`);
      }
  ...
  function addHooks () {
    browserify.pipeline.get('pack').push(through(function write (row, enc, next) {
      ...
      const csspackingtime = process.hrtime();
      ...
      Promise.all(writes)
        .then(function () { 
          if(transformOpts.verbose) {
            var hrend = process.hrtime(csspackingtime);
            console.log(`${colors.yellow("css-modulesify")}-${colors.magenta("  packing")}: ${colors.green(cssOutFilename)} -> ${ `${colors.cyan(hrend[1]/1000000)}ms` }}`);
          }
          cb(); 
        })
